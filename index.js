...
function findLargestInteger(integers) {
  ...
  for(let i = 0; i < integers.length; i++) {
    if(integers[i].integer > maxValue) {
      maxValue = integers[i].integer;
    }
  }
  return maxValue;
}
...
class Seasons {
   ...
   */
  next() {
    const currSeason = this.seasons[this.count];
    if (this.count === 3) {
      this.count = 0;
    } else {
      ++this.count;
...
class Car {
   ...
   */
  drive(distance) {
    const reduceGas = distance / this.mpg;
    this.odometer += distance;
    if (reduceGas >= this.tank) {
      this.tank = 0;
    } else {
      this.tank -= reduceGas;
    }
    if (this.odometer > this.tank * this.mpg) {
